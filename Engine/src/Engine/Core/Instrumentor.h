	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					ENGINE_ERROR("Instrumentor::BeginSession('{}') when session '{}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				...
				{
					ENGINE_ERROR("Instrumentor could not open results file '{}'.", fullFilePath.string());
				}
			}
		}
		...
		void WriteProfile(const ProfileResult& result)
		{
			...
			else
			{
				...
				if (Engine::Log::GetCoreLogger())
				{
					ENGINE_ERROR("Attempted to write profile '{}' without active session", result.Name);
				}
			}
		}
